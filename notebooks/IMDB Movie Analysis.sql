...
SELECT
          ...
          SUM(CASE WHEN title IS NULL THEN 1 ELSE 0 END) AS title_nulls,
           ...
           SUM(CASE WHEN counrty IS NULL THEN 1 ELSE 0 END) AS country_nulls,
...
-- Type your code below:
select year, genre, count(movie_id) as number_of_movies
from genre as g
...
inner join ratings as r
on m.id = r.movie_id
where avg_rating >8 and prod_company_rank is not null
...
select production_company, sum(total_votes) as vote_count,
                         dense_rank() over(order by sum(total_votes) as prod_comp_rank
  from movies as m
  inner join ratings as r
  on m_id = r.movie_id
  group by production_company
...
+---------------+-------------------+---------------------+----------------------+-----------------+*/
-- Type your code below:
select distinct name as actor_name,   total_votes, count(movie_id) as movie_count,
...
+---------------+-------------------+---------------------+----------------------+-----------------+*/
-- Type your code below:
select name as actress_name, total_votes, count(movie_id) as movie_count,
...
on rm.name_id= nm.id
where country ='India' and category= 'Actress' and language='Hindi'
group by name
having count(m.id)>=3
limit 1;
...
+---------------+-------------------+---------------------+----------------------+*/
-- Type your code below:
select genre, round(av(duration),2) as avg_duration,
...
(
   select d.name_id, name, d.name_id, m.date_published,
